...
test_r_api <- function(folder, input4, input2, input3, output1, output2) {
  ...
  msg = paste0("Saved local file: ", output1, " to ", output1, " with content: Test output1")
  faasr_log(msg)
  
  # Test putting output2
  writeLines("Test output2", output2)
  faasr_put_file(local_file=output2, remote_folder=folder, remote_file=output2)
  msg = paste0("Saved local file: ", output2, " to ", output2, " with content: Test output2")
  faasr_log(msg)
